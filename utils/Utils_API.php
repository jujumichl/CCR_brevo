...
class Adherents{
    ...
    function addContact(string $csvContent, string $apikey, int $listId): array {
        ...
        $request = [
            ...
            "fileBody" => $csvContent,       // Contenu CSV en texte brut (pas base64)
    ...
    function checkContact(array $csvContent, array $emails, array $noms, array $prenoms){
        ...
        $chemin = dirname(__DIR__, 1) . "\\fichier\\verification.csv";
        if (!empty($erreur)) {
            ecrireCsv( $chemin,$erreur, ['Nbr', 'Nom', 'Prénom', 'Portable', 'Courriel', 'Année'], ';');
        }
        return $erreur;
    }
